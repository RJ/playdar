...
e.src = 'http://www.playdar.org/static/playdar.js?greasemonkey'; // same js is served, this is jsut for log-grepping ease.
...
function go() {
  var playdar = unsafeWindow.playdar;
...
function do_parsing(playdar){
  ...
  for(var i = 0; i<links.length; i++){
    ...
    if(url.directoryPath.substr(0,7)!="/music/") continue;
    ...
    if(links[i].getAttribute('class') != null) continue;
...
function do_markup(playdar, a, artist, track){
	...
	if(response.results.length){
	    ...
	    unsafeWindow.document.getElementById(response.qid).innerHTML = "&nbsp;<a href=\""+playdar.sid2url(response.results[0].sid)+"\" title=\""+tt+"\">" + response.results.length + "</a>&nbsp;";
	}else if(finalanswer){
